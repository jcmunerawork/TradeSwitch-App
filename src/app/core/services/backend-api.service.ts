/**
 * Servicio de API del backend (TradeSwitch).
 *
 * Centraliza todas las peticiones HTTP al backend: autenticación, usuarios,
 * cuentas, estrategias, suscripciones, TradeLocker, reportes, planes, etc.
 * Extiende BaseApiService y usa el token de Firebase en el header Authorization.
 *
 * Uso: inyectar BackendApiService y llamar a los métodos async (devuelven Promise).
 * La URL base se obtiene de ConfigService (apiUrl) + '/v1'.
 */
...
/**
 * Envoltorio estándar de respuesta del backend (success, data, error, message).
 */
...
/**
 * Servicio que realiza todas las llamadas al backend (auth, users, accounts, strategies, etc.).
 * Requiere token de Firebase (idToken) en los métodos que usan Authorization.
 */
@Injectable({
  providedIn: 'root'
})
export class BackendApiService extends BaseApiService {
  /** Prefijo de la API en la URL base (ej. /api + /v1). */
  protected apiUrl = '/v1';
